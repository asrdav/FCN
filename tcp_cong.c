...
{
	//printk("\nRENO: tcp_set_congestion_control");
	struct inet_connection_sock *icsk = inet_csk(sk);
	const struct tcp_congestion_ops *ca;
	int err = 0;
...
u32 tcp_reno_ssthresh(struct sock *sk)
{
	...
	int updatedFactor = 9;  //updateFactor value range from 5 to 9.
	int newCwnd = (tp->snd_cwnd * updatedFactor)/10;
	printk("\nRENO: newCwnd: %d",newCwnd);
	return max(newCwnd, 2);
}
